...
{
  double dt;
  int L, K, nptsa, nptsr, nptsi;
  string tensor_file;

  po::options_description options("options");
  options.add_options()
      ("help", "produce help message")
      (",K", po::value<int>(&K)->default_value(2), "Polynomial degree K")
  ...
  typedef collision_tensor_assembly::
      CollisionOperator<basis_type, KERNEL_TYPE::MAXWELLIAN, QMidpoint, QRadial>
          collision_operator_t;

  CollisionTensorExporter exporter("collision_tensor.h5");

  collision_operator_t collision_operator(test_basis, trial_basis, nptsa, nptsr, nptsi);
