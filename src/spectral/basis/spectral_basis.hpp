...
class SpectralBasis
{
 ...
 public:
  ...
  SpectralBasis()
      : counter(0){};
  //  SpectralBasis& operator=(const SpectralBasis& other);
  ...
  bool operator==(const SpectralBasis<E>& other);
...
template <typename E>
bool
SpectralBasis<E>::operator==(const SpectralBasis<E> &other)
{
  if (this->size() != other.size()) return false;

  for (int i = 0; i < this->size(); ++i) {
    if (not (get_elem(i).id() == other.get_elem(i).id())) return false;
  }
  return true;
}
