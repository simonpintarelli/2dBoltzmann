...
class SpectralBasisFactoryKS : public SpectralBasisFactoryBase<XiR, LaguerreKS>
{
 public:
  /**
   *  @brief create the Polar-Laguerre basis with polynomial degree <K, sort basis by (l, t, k)
  ...
  static void create_test(basis_type &basis, int K);

  static basis_type create(int K);
...
// ----------------------------------------------------------------------
inline SpectralBasisFactoryKS::basis_type
SpectralBasisFactoryKS::create(int K)
{
  basis_type basis;
  double beta = 2;
  bool sorted = true;
 ...
 *  @param L max. angular part
 *  @param beta temperature
 *  @param sorted if true, sort by l
 */
inline void
 ...
 */
inline void
SpectralBasisFactoryKS::create_test(basis_type& basis, int K)
{
  create_test(basis, K, K, 2., true);
}
