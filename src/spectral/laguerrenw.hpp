...
#pragma omp parallel for
    for (size_t xi = 0; xi < n; ++xi) {
      ...
      Y_[alpha][0][xi] = bm::laguerren(0, alpha, x[xi]) * expw;
      Y_[alpha][1][xi] = bm::laguerren(1, alpha, x[xi]) * expw;
    }
...
#pragma omp parallel for
      for (size_t xi = 0; xi < n; ++xi) {
        Y_[alpha][k][xi] =
            bm::laguerren_next(k - 1, alpha, x[xi], Y_[alpha][k - 1][xi], Y_[alpha][k - 2][xi]);
      }
    }
  }
}
