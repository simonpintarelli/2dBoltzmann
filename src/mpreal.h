...
// MPFR 2.4.0 Specifics
  ...
  friend const mpreal rem(const mpreal& x,
                          const mpreal& y,
                          mp_rnd_t rnd_mode);  // Remainder after division
  friend const mpreal mod(const mpreal& x,
                          const mpreal& y,
                          mp_rnd_t rnd_mode);  // Modulus after division
#endif
...
// MPFR < 2.4.2 Specifics
  ...
  inline void set_prec(
      mp_prec_t prec,
      mp_rnd_t rnd_mode = get_default_rnd());  // Change precision with rounding mode

  // Aliases for get_prec(), set_prec() - needed for compatibility with std::complex<mpreal>
  // interface
...
{
  mpfr_mul_2ui(
      mpfr_ptr(), mpfr_srcptr(), static_cast<unsigned long int>(u), mpreal::get_default_rnd());
  MPREAL_MSVC_DEBUGVIEW_CODE;
  return *this;
}
...
operator!=(const mpreal& a, const mpreal& b)
{
  return (mpfr_lessgreater_p(a.mpfr_srcptr(), b.mpfr_srcptr()) != 0);
}

inline bool
operator==(const mpreal& a, const unsigned long int b)
{
  return (mpfr_cmp_ui(a.mpfr_srcptr(), b) == 0);
}
inline bool
...
{
  return (mpfr_cmp_si(a.mpfr_srcptr(), b) == 0);
}
inline bool
...
{
  return (mpfr_cmp_ld(a.mpfr_srcptr(), b) == 0);
}
inline bool
operator==(const mpreal& a, const double b)
{
  return (mpfr_cmp_d(a.mpfr_srcptr(), b) == 0);
}
...
mpreal::output(std::ostream& os) const
{
  ...
  char* s = NULL;
  if (!(mpfr_asprintf(&s, format.str().c_str(), mpfr::mpreal::get_default_rnd(), mpfr_srcptr()) <
        0)) {
    os << std::string(s);
    mpfr_free_str(s);
  }
...
inline const mpreal
frexp(const mpreal& v, mp_exp_t* exp)
{
  mpreal x(v);
  *exp = x.get_exp();
  x.set_exp(0);
...
inline const mpreal
remquo(long* q, const mpreal& x, const mpreal& y, mp_rnd_t rnd_mode = mpreal::get_default_rnd())
{
  mpreal a(0, (std::max)(y.getPrecision(), x.getPrecision()));
...
inline const mpreal
sum(const mpreal tab[], unsigned long int n, mp_rnd_t rnd_mode = mpreal::get_default_rnd())
{
  mpreal x;
  mpfr_ptr* t;
  unsigned long int i;
...
pow(const unsigned int a, const unsigned int b, mp_rnd_t rnd_mode)
{
  return pow(static_cast<unsigned long int>(a),
             static_cast<unsigned long int>(b),
...
class numeric_limits<mpfr::mpreal>
{
 public:
  ...
  MPREAL_PERMISSIVE_EXPR static const int min_exponent10 =
      (int)(MPFR_EMIN_DEFAULT * 0.3010299956639811);
  MPREAL_PERMISSIVE_EXPR static const int max_exponent10 =
      (int)(MPFR_EMAX_DEFAULT * 0.3010299956639811);
  ...
  // But it is incompatible with standard std::numeric_limits and might not work with other
  // libraries, e.g. boost.
  ...
  // max_digits10 = 78
  //
  // Approximate formula for decimal digits is: digits10 = floor(log10(2) * digits). See
