...
#include "ct_input.hpp"

namespace boltzmann {
namespace ct_dense {

class CollisionTensorZLastAMEigen : public CollisionTensorZLastAMstorage
{
 public:
  typedef std::vector<double, boost::alignment::aligned_allocator<double, 32>> aligned_vector;

 public:
  CollisionTensorZLastAMEigen(const basis_t &basis, int bufsize)
   ...
   * @param[in]  imax apply slices up to and including index imax, if <= 0, apply all
   */
...
template <typename DERIVED1, typename DERIVED2>
void
CollisionTensorZLastAMEigen::apply(Eigen::DenseBase<DERIVED1> &out,
                                   const Eigen::DenseBase<DERIVED2> &in,
                                   int imax) const
{
  ...
  typedef Eigen::Map<mat_t, Eigen::Aligned32, Eigen::OuterStride<>> map_t;
  typedef Eigen::Map<const mat_t, Eigen::Aligned32, Eigen::OuterStride<>> cmap_t;
    ...
    for (unsigned int z = 0; z < vbcrs.dimz(); ++z) {
      ...
      for (unsigned int i = 0; i < vbcrs.nblock_rows(); ++i) {
        ...
        for (unsigned int blkidx = vbcrs.row_begin(i); blkidx < vbcrs.row_end(i); ++blkidx) {
          ...
          cmap_t A(data + memoffset, row_extent, col_extent, Eigen::OuterStride<>(padded_row_size));
          cmap_t B(x1, col_extent, in.cols(), Eigen::OuterStride<>(Npadded));
          map_t C(buf + padded_row_offset, row_extent, in.cols(), Eigen::OuterStride<>(Npadded));
          C.noalias() += A * B;
      ...
      for (int kk = 0; kk < in.cols(); ++kk) {
        Eigen::Map<Eigen::VectorXd> vb(buf + kk * Npadded, Npadded);
        Eigen::Map<const Eigen::VectorXd> vi(in.derived().data() + kk * Npadded, Npadded);
        out(slice_id, kk) = sinv * vb.dot(vi);
      }
    }  // end iterate over z
    // update z-offset
    z_offset += vbcrs.dimz();
