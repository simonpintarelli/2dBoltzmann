...
private:
  typedef Loss<BASIS_TYPE> loss_t;
...
public:
    ...
    beta(beta_)
  {
    ...
    loss.init(&trial_basis_, &(var_form.s0()));
        ...
        const unsigned int j)
{
  ...
  const double kernel = 1.0/(2*numbers::PI);
  const auto& test_elem = test_basis.get_elem(j);
  gain_t gain(test_elem, inner_product_weight, nptsi);

  const auto& id = test_elem.get_id();
  // inner factor
  // const int t = boost::fusion::at_key<angular_elem_t>(id).t;
  // const int l = boost::fusion::at_key<angular_elem_t>(id).l;
  // gcc 5.1 does not like const refs in lambdas inside function templates...
  ...
  for (unsigned int q1 = 0; q1 < quad.size(); ++q1) {
    ...
    // I^+(v, v_*) is symmetric for Maxwellian molecules!
    for (unsigned int q2 = q1; q2 < quad.size(); ++q2) {
      // v*
      ...
      if ( q1 == q2) {
        innerMX[q1][q2] = kernel*prefactor(alpha)*res;
      } else {
        innerMX[q1][q2] = kernel*prefactor(alpha)*res;
        innerMX[q2][q1] = kernel*prefactor(alpha)*res;
      }
    }
  }
           ...
           const unsigned int j)
{
  ...
  const auto& id = test_elem.get_id();
  //const int l = boost::fusion::at_key<angular_elem_t>(id).l;
  // gcc 5.1 does not like ref's to const int's inside lambdas inside function templates
  int l = boost::fusion::at_key<angular_elem_t>(id).l;
  Timer<> timer;
  ...
  auto check_sparsity = [&](int l1, int l2) {
    if ( std::abs(l1+l2) == l || std::abs(l1-l2) == l)
      return true;
    else
      return false;
  };
  ...
  for (unsigned int l1 = 0; l1 < Loffsets.size()-1; ++l1) {
    ...
    Eigen::Map<const matrix_t> B1(basis_functions[Loffsets[l1]].origin(),
                                  k1size, /* rows */
                                  Nq /* cols */);
    for (unsigned int l2 = l1; l2 < Loffsets.size()-1; ++l2) {
      ...
      if (check_sparsity(l1, l2)) {
        ...
        for (unsigned int j1 = Loffsets[l1]; j1 < Loffsets[l1+1]; ++j1) {
          for (unsigned int j2 = Loffsets[l2]; j2 < Loffsets[l2+1]; ++j2) {
            ...
            if (l1 == l2)  {
              storage.insert(j1, j2, res(i1, i2) - loss.compute(j, j1, j2));
            } else {
              storage.insert(j1, j2, res(i1, i2) - loss.compute(j, j1, j2));
              storage.insert(j2, j1, res(i1, i2) - loss.compute(j, j2, j1));
            }
          }
        }
      }
